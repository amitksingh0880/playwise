...
function getOrCreatePeer(targetId) {
    ...
    const entry = { pc, iceCandidateQueue: [], makingOffer: false, ignoreOffer: false };
    ...
    pc.onicecandidate = ({ candidate }) => {
        if (candidate)
            chrome.runtime.sendMessage({ type: "SEND_SIGNAL", targetId, signalType: "webrtc-ice", candidate });
    };
...
function renderCams() {
    ...
    Object.entries(peers).forEach(([peerId, entry]) => {
        const remoteStream = entry.pc._remoteStream;
